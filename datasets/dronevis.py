...
class DroneDetection(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, cache_mode=False, local_rank=0, local_size=1):
        super(DroneDetection, self).__init__(img_folder, ann_file,
                                            cache_mode=cache_mode, local_rank=local_rank, local_size=local_size)
        self._transforms = transforms
        self.prepare = ConvertCocoPolysToMask(return_masks)

    def __getitem__(self, idx):
        img, target = super(DroneDetection, self).__getitem__(idx)
...
def make_drone_transforms(image_set):
...
def build(image_set, args):
    ...
    PATHS = {
        ...
        "test": (root / "visDrone2019_test/test", root / f'visDrone2019_test/visDrone_test.json')
    }

    img_folder, ann_file = PATHS[image_set]
    dataset = DroneDetection(img_folder, ann_file, transforms=make_drone_transforms(image_set), return_masks=args.masks,
...
# class DroneDetection(torchvision.datasets.CocoDetection):
